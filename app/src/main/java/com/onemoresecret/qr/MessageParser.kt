...
abstract class MessageParser {
    ...
    fun consume(qrCode: String) {
        ...
        if (OneTimePassword(qrCode).isValid) {
        ...
        val data = text.substring(0, chunkSize) //remove padding
        ...
        val cntReceived =
            chunks.stream().filter { obj: String? -> Objects.nonNull(obj) }.count().toInt()
        ...
        if (cntReceived == chunks.size) {
            ...
            //remove line breaks
            val msg = java.lang.String.join("", chunks).replace("[\\r\\n]".toRegex(), "")
            transactionId = null
            onMessage(msg)
        }
    }
