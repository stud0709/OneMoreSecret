...
class CryptographyManager {
    ...
    fun getPrivateKey(keyName: String?): PrivateKey? {
        ...
        return if (key == null) null else key as PrivateKey
    }
    ...
    /**
     * Import RSA key pair using encoded key data from [PrivateKey.getEncoded].
     */
    @Throws(
    ...
    fun importKey(keyName: String, keyPair: KeyPair, ctx: Context) {
        ...
        val keyProtection =
            KeyProtection.Builder(KeyProperties.PURPOSE_ENCRYPT or KeyProperties.PURPOSE_DECRYPT)
                ...
                .setIsStrongBoxBacked(ctx.packageManager.hasSystemFeature(PackageManager.FEATURE_STRONGBOX_KEYSTORE))
     ...
     */
    @Throws(KeyStoreException::class)
    fun getByFingerprint(fingerprint: ByteArray): List<String> {
        ...
        while (aliases.hasMoreElements()) {
            ...
            try {
                ...
                val _fingerprint = RSAUtils.getFingerprint(publicKey)

                if (fingerprint.contentEquals(_fingerprint)) {
                    result.add(alias)
                }
            } catch (e: Exception) {
    ...
    companion object {
        ...
        val ENCRYPTION_PADDINGS: Array<String> = arrayOf(
        ...
        fun generateKeyPair(keyLength: Int): KeyPair {
            ...
            val privateKeyMaterial = privateKeyInfo.encoded
            val publicKeyMaterial =
                SubjectPublicKeyInfoFactory.createSubjectPublicKeyInfo(asymmetricCipherKeyPair.public).encoded
