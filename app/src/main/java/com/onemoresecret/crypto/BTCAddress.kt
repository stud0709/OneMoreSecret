...
object BTCAddress {
    ...
    private val toByte32 =
        Function { bi: BigInteger ->
         ...
         * or shorter than 32 bytes. We must handle both cases */
            ...
            System.arraycopy(
                ...
                max(
                    0.0,
                    (src.size - dest.size).toDouble()
                ).toInt(),  //skip leading zeros if longer than 32 bytes
                dest,
                max(
                    0.0,
                    (dest.size - src.size).toDouble()
                ).toInt(),  //prepend with leading zeros if shorter than 32 bytes
                min(src.size.toDouble(), dest.size.toDouble()).toInt()
            )
            dest
        }

    private val sha256 =
        Function { bArr: ByteArray ->
            try {
    ...
    fun validateWIF(wifString: String): Boolean {
        ...
        //drop last 4 bytes
        val withQualifier = Arrays.copyOfRange(wif, 0, wif.size - 4)

        //calculate dual sha256
        val digest = sha256.apply(sha256.apply(withQualifier))

        //compare checksums
        return Arrays.copyOfRange(digest, 0, 4).contentEquals( //first 4 bytes of the digest
            Arrays.copyOfRange(wif, wif.size - 4, wif.size) //last 4 bytes of WIF byte array
        )
    }
    ...
    fun toPrivateKey(wif: ByteArray): ByteArray {
        ...
        return Arrays.copyOfRange(wif, 1, wif.size - 4)
    }
