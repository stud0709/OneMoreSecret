...
class OneTimePassword(s: String) {
    ...
    val secret: String
        get() = uri.getQueryParameter(SECRET_PARAM)!!.uppercase(Locale.getDefault())

    val period: Int
        get() {
            val s = uri.getQueryParameter(PERIOD_PARAM)
            return if (s.isNullOrEmpty()) DEFAULT_PERIOD else s.toInt()
        }
    ...
    val digits: Int
        ...
        get() {
            ...
            val d = if (s == null || s.isEmpty()) DIGITS[0] else s
            require(
                Arrays.stream(DIGITS)
                    .anyMatch { anObject: String? -> d.equals(anObject) }) { "Invalid digits: $d" }
            return d.toInt()
        }
    ...
    val algorithm: String
        get() {
            ...
            val alg = if (s == null || s.isEmpty()) ALGORITHM[0] else s
            require(
                Arrays.stream(ALGORITHM)
                    .anyMatch { anObject: String? -> alg.equals(anObject) }) { "invalid algorithm: $alg" }
            return alg
        }
    ...
    fun sign(data: ByteArray?): ByteArray {
        ...
        mac.init(SecretKeySpec(decode(secret), ""))
    ...
    fun generateResponseCode(challenge: ByteArray?): String {
        ...
        val pinValue = truncatedHash % BigDecimal.TEN.pow(digits).toInt()
    ...
    val state: OtpState
        get() {
            ...
            return OtpState(now_s / period, now_s % period)
        }
    ...
    companion object {
        ...
        @JvmField
        val DIGITS: Array<String> = arrayOf("6", "8")
        const val DIGITS_PARAM: String = "digits"
        const val ALGORITHM_PARAM: String = "algorithm"
        @JvmField
        val ALGORITHM: Array<String> = arrayOf("SHA1", "SHA256", "SHA512")
